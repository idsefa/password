...
class PasswordGeneratorApp(QWidget):
    ...
    def init_ui(self):
        ...
        self.password_length_label = QLabel("密码长度:")
        self.password_length_input = QLineEdit()
        self.password_length_input.setValidator(QRegExpValidator(QRegExp("[1-9][0-9]{0,2}"), self))  # 限制输入为正整数
        self.password_length_input.setText("8")
    ...
    def generate_password_logic(self, site_name, username, include_uppercase, include_special_chars, extra_seed=""):
        # 检查是否有重复的数据
        if self.check_duplicate(site_name, username, extra_seed):
            QMessageBox.warning(self, '警告', '数据库中已存在相同网站、用户名和种子值的记录！')
            return None

        # 检查网站和用户名是否为空
        if not site_name or not username:
            QMessageBox.warning(self, '警告', '网站和用户名不能为空！')
            return None
        
        # 获取用户指定的密码长度
        password_length = int(self.password_length_input.text())
        
        # 使用网站或应用程序名称和用户名以及额外种子作为种子
        seed_string = site_name + username + extra_seed
        random.seed(seed_string)

        # 生成基本密码
        base_password = ''.join(random.choices(string.ascii_lowercase, k=password_length))
        ...
        if include_special_chars:
            base_password += random.choice(string.punctuation)

        # 获取当前时间戳并添加到密码中
        timestamp = int(datetime.timestamp(datetime.now()))
        base_password += str(timestamp)
        ...
        # 将密码信息插入数据库
        self.cursor.execute('''
            INSERT INTO passwords (site_name, username, password, timestamp, seed_used)
            VALUES (?, ?, ?, ?, ?)
        ''', (site_name, username, password, timestamp, extra_seed))
        self.conn.commit()

        return password
    
    def check_duplicate(self, site_name, username, extra_seed):
        # 检查数据库中是否有相同的网站、用户名和种子值记录
        self.cursor.execute('''
            SELECT * FROM passwords
            WHERE site_name = ? AND username = ? AND seed_used = ?
        ''', (site_name, username, extra_seed))
    ...
    def generate_password(self):
        ...
        # 获取额外种子值
        extra_seed, ok = QInputDialog.getText(self, "额外种子", "请输入额外种子值 (默认为空):", QLineEdit.Normal)
        ...
        # 使用密码生成函数
        generated_password = self.generate_password_logic(site_name, username, include_uppercase, include_special_chars, extra_seed)
